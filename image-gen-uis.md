...
```bash
python launch.py --listen --port 7860 --gradio-auth "admin:REPLACE_WITH_LONG_RANDOM_VALUE"
```
...
The `Web Server` section of `invokeai.yaml` sets `host` (default `127.0.0.1`) and `port` (default `9090`); setting `host: 0.0.0.0` serves the local network with no login at all in the default single-user mode. `INVOKEAI_HOST` and `INVOKEAI_PORT` override the same settings from the environment.
...
- InvokeAI Multi-User Administrator Guide: http://invoke.ai/features/multi-user-mode/admin-guide/
