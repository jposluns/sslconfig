...
The moment that route exists the app is reachable at `https://app.example.com`, by anyone, with no login in front of it. Access is what adds the login and it is step 4, so doing these steps in the order they are numbered leaves a window in which the application is published and unauthenticated, however short you make it. Close it rather than racing it: complete step 4 first and add the route afterwards, or leave the local service stopped until Access is in place, so that the route resolves to nothing until there is something guarding it. [deployment-lifecycle.md](deployment-lifecycle.md) makes the same point about previews and first deployments.
