...
Run a current PgBouncer. 1.25.2 fixes four advisories, three of which an unauthenticated remote client can reach: an integer overflow in packet parsing that crashes the process from a malformed SCRAM packet (CVE-2026-6664), a SCRAM `strlcat` return value that a malicious backend can exploit (CVE-2026-6665), a null dereference on a server error response with no SQLSTATE (CVE-2026-6666), and a missing authorization check that let any console user, including a read-only one, run `KILL_CLIENT` (CVE-2026-6667). The last one matters to section 6 below, which treats `stats_users` as read-only; on an older build it is not.
...
Pgpool-II uses one switch for both hops: `ssl` is "off" by default, and setting it on "enables the SSL for both the frontend and backend communications". Its documentation notes that `ssl_key` and `ssl_cert` must also be configured for frontend connections to work. Turning SSL on only makes TLS available; it does not refuse a client that declines it. Refusing one is `pool_hba.conf`, in section 5.
...
A line such as `hostssl app app 10.0.0.0/24 scram-sha-256`, which used to restrict a range of clients, now restricts one host. That is not a reason to loosen it, and narrowing it is not the whole job either: installing a pooler does not stop anything else in that range connecting directly. Narrow the database server's rules to the address you just read, and put the client-facing restriction where the clients now arrive.
...
Pgpool-II has the same file and does not read it by default: `enable_pool_hba` is documented as "Default is false", and with it off "the client authentication method is completely managed by PostgreSQL", which after section 4 means it is not discriminating between your clients at all.
...
The client hop takes two commands, not one. A single failing connection proves nothing, because a wrong password fails exactly the same way as a refused plaintext connection. Run the pair with the same account and require one of each result:

```bash
PGPASSWORD=REPLACE_WITH_THE_APP_PASSWORD
export PGPASSWORD

# Must FAIL, and the error must name TLS rather than authentication.
psql "host=pooler.internal port=6432 dbname=app user=app sslmode=disable" -c 'SELECT 1;'

# Must SUCCEED. This is the control: it proves the credential is good, so the
# failure above was the TLS requirement and not a bad password.
psql "host=pooler.internal port=6432 dbname=app user=app sslmode=verify-full sslrootcert=/etc/ssl/certs/ca.crt" -c 'SELECT 1;'
```
...
`ssl` reports whether the pooler-to-PostgreSQL connection uses SSL. It does not report whether the pooler validated the certificate, so a `t` here is consistent with `server_tls_sslmode = require`, which validates nothing. Certificate validation cannot be observed from the client at all: the only test is to present the pooler with a certificate that should fail, one signed by an untrusted CA and one valid but issued for a different host name, and confirm on a fresh connection that it refuses both. `inet_client_addr()` is the address section 5 tells you to write into `pg_hba.conf`.
...
That has to fail while connecting, with `not allowed`. A failure at the command instead means the login succeeded, which is what `auth_type = any` produces: the connection is admitted as a stats user and only privileged commands are refused. If the connection is accepted at all, check `auth_type` before anything else.
